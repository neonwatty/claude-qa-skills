...
- description: "Screenshot each workflow's starting page before any steps execute"
...
2. Wait for the page to fully load: `computer({ action: 'wait', duration: 2, tabId })`
3. Take a screenshot using `computer({ action: 'screenshot', tabId })`
...
└── before-after-report.html           (side-by-side comparison report)
```
...
- If PASS: Proceed to after-state capture and report generation
...
**Purpose:** Generate a standalone, self-contained HTML report showing side-by-side before/after screenshots for each workflow — showing the overall transformation at a glance.
...
- subagent_type: "general-purpose"
- prompt: |
    Generate a before/after HTML report for browser workflow testing.
    ...
    - Read the findings from task list or `.claude/plans/browser-workflow-findings.md` for issue counts

    ## Output
    Write to `workflows/before-after-report.html`

    ## Requirements
    - Self-contained HTML (all images embedded as base64 using: base64 -i <file>)
    - Side-by-side comparison layout for each workflow
    - Executive summary: workflows tested, issues found, issues fixed
    ...
    <html>
    <head>
      <title>Before/After Report — Browser Workflows</title>
      <style>
        body { font-family: system-ui; max-width: 1200px; margin: 0 auto; padding: 2rem; }
        .summary { background: #f5f5f5; padding: 1.5rem; border-radius: 8px; margin-bottom: 2rem; }
        .workflow-section { margin-bottom: 3rem; border-bottom: 1px solid #eee; padding-bottom: 2rem; }
        .comparison { display: flex; gap: 2rem; align-items: flex-start; }
